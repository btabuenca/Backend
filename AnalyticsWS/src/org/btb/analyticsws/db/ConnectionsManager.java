...
public class ConnectionsManager {
   ...
   *
   * @exception DbConnectException Si se no se puede crear el gestor.
   */
  ...
  /**
   * Metodo que lee el fichero de configuracion. Inicializa los atributos que controlan
   * las trazas, los errores de log, y llama a los metodos para cargar los drivers de
   * conexion a las bases de datos, y para la creacion de pools.
   *
   * @exception DbConnectException Si falla la carga de algun atributo.
   */
   ...
   *
   * @exception DbConnectException Si falla la carga de los drivers desde
  ...
  private void loadDrivers() throws DbConnectException {
    ...
    try {
      ...
      log.fine("Testing logger 1");
      Driver driver = (Driver) Class.forName(driverName).newInstance();
      log.fine("Testing logger 2");
      DriverManager.registerDriver(driver);
      log.fine("Testing logger 3");
      drivers.addElement(driver);
      log.fine("Testing logger 4");
    } catch (Exception e) {
    	e.printStackTrace();
  ...
  /**
   * Metodo que crea los pools de conexiones leyendo para cada pool la url del servidor,
   * el login, la password, y el numero maximo de conexiones de ficheros de configuracion.
   * Introduce el pool creado en una tabla Hash de pools.
   *
   * @exception DbConnectException Si falla la creacion del pool de conexiones.
   */
  private void createPools() throws DbConnectException {


//	  DRIVER_JDBC=oracle.jdbc.driver.OracleDriver
//	  URL=jdbc:oracle:thin:@10.254.222.73:1521:OR1
//	  LOGIN=cbarreras
//	  PASSWORD=cbarreras
//	  MAX_CONNECTION=300
...
//    String maxConn = "300";
  ...
  /**
   * Metodo que devuelve el pool correspondiente al nombre pasado como parametro. El pool
   * es buscado en la tabla Hash de pools.
   * @param name String Nombre del pool
  ...
  /**
   * Metodo que devuelve el pool correspondiente al nombre y al tiempo de conexion, que
   * se pasan como parametros.
   * @param time long Tiempo de la conexion
  ...
  /**
   * Metodo que libera la conexion indicada del pool correspondiente a ese nombre.
   * @param con Connection Conexion que se de sea liberar
   */
